...
abstract class Map implements ActiveRecordInterface
{
    ...
    public function setDescription($v)
    {
        ...
        if ($this->description !== $v) {
            $this->description = $v;
            $this->modifiedColumns[MapTableMap::COL_DESCRIPTION] = true;
        }
    ...
    public function setImageFile($v)
    {
        ...
        if ($this->image_file !== $v) {
            $this->image_file = $v;
            $this->modifiedColumns[MapTableMap::COL_IMAGE_FILE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : MapTableMap::translateFieldName('Description', TableMap::TYPE_PHPNAME, $indexType)];
            $this->description = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : MapTableMap::translateFieldName('ImageFile', TableMap::TYPE_PHPNAME, $indexType)];
            $this->image_file = (null !== $col) ? (string) $col : null;
            $this->resetModified();
            ...
            return $startcol + 4; // 4 = MapTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(MapTableMap::COL_DESCRIPTION)) {
            $modifiedColumns[':p' . $index++]  = 'description';
        }
        if ($this->isColumnModified(MapTableMap::COL_IMAGE_FILE)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'description':
                        ...
                        break;
                    case 'image_file':
                        $stmt->bindValue($identifier, $this->image_file, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(MapTableMap::COL_DESCRIPTION)) {
            $criteria->add(MapTableMap::COL_DESCRIPTION, $this->description);
        }
        if ($this->isColumnModified(MapTableMap::COL_IMAGE_FILE)) {
            $criteria->add(MapTableMap::COL_IMAGE_FILE, $this->image_file);
        }
